...
/* JavaScript content from js/LoginRealmChallengeProcessor.js in folder common */
/*
...
LoginRealmChallengeHandler.handleChallenge = function(response) {
	...
	if (authRequired == true) {
		if (response.responseJSON.errorMessage
			...
			cordova.exec(DeviceSuccess,DeviceFailure,"PushPlugin","deviceID",["test"]);
		...
		ADF.user = {
			...
			tokenID: response.responseJSON.tokenID
		};
		
// Encryted Cache token id ...		
//		WL.EncryptedCache.read("test", onDecryptReadSuccess, onDecryptReadFailure);
//		function onDecryptReadSuccess(value){
//			alert("Read success. Retrieved value :: " + key + " = " + value); 
//			if(value===null||value===""){
//				WL.EncryptedCache.write("test", ADF.user.tokenID, onWriteSuccess, onWriteFailure);
//				function onWriteSuccess(status){
//					alert("Succesfully encrypted into cache.");
//					if (ADF.user.tokenID) {
//						cordova.exec(plSuccess,plFailure,"PushPlugin","tokenID",[ADF.user.tokenID]);
//					}
//				}
...
//				}
//			}else if(value===ADF.user.tokenID){
//				alert("cash data ==ADF.user.tokenID"+value);
//				
//			}else if(value!==ADF.user.tokenID){
//				alert("value!== delelte and write");
//				WL.EncryptedCache.remove("test", onRemoveSuccess, onRemoveFailure);
...
//		}
		...
		//token Id ... 
		cordova.exec(plugInSuccess,pluginFailure,"PushPlugin","loginAction",[ADF.user.tokenID,ADF.user.nameen]);
		
		
		function plugInSuccess(data){
			console.log(data);
		}
		
		function pluginFailure(data){
			console.log(data);
		}
...
function plSuccess(){
	WL.Logger.debug("plugin call success...");
}


function plFailure(){
	WL.Logger.debug("plugin call plFailure...");
}
