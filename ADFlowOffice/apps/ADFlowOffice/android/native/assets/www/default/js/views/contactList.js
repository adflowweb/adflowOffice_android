...
/* JavaScript content from js/views/contactList.js in folder common */
//JSONStore start
		...
		.extend({
			...
			loadList : function() {
				...
				var males = [ 'eugene_lee', 'gary_donovan', 'james_king',
						'john_williams', 'paul_jones', 'ray_moore',
						'steven_wells' ];
				...
				for (var i = 0; i < this.collection.length; i++) {
					...
					if (this.collection.models[i].get('photo').length == 0) {
						...
						console.log('in else...photo start......');
						console.log(this.collection.models[i].get('photo'));
						console.log("photo_chan  photo_chanLength  ::" + this.collection.models[i].get('photo').length);
						console.log(this.collection.models[i].get('nameKo'));
						console.log('in  else...photo end......');
						
					
						liSrc += this.constant1
								+ this.collection.models[i].get('phone')
								+ '" src="'
								+ "data:image/png;base64,"+this.collection.models[i].get('photo')
								+ '" /></div>'
								+ '<div style="color:grey;float:left"><h3 style="padding-left:10px;">'
								+ this.collection.models[i].get('nameKo')
								+ '<font style="font-size:15px">&nbsp;&nbsp;'
						...
						.on(
								{
									...
									click : function(e) {
										...
										ADF.view.contantDetail.addFlagOn(true);
										navigation
												.pushView(
														ADF.view.contantDetail,
														'typeA');
			...
			loadContactList : function(param, orchestrationName) {
				...
				//param 	that.loadContactList('{"act" : "R"}', "ADFlowContact");
